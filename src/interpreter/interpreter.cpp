...
void expression_evaluator::visit(function_call_expression const& node) {
    ...
    if (*callee->identifier.payload == "printf") {
        ...
        expression_evaluator eval;
        node.arguments.front()->accept(eval);
        if (!std::holds_alternative<std::string>(eval.result)) {
            std::cerr << "Interpreter error: Function printf requires a format string as its first argument\n";
            return;
        }
        auto format = std::get<std::string>(eval.result);
        std::cout << format << " ";

        // TODO: Use format string to print arguments
        for (auto it = node.arguments.begin() + 1; it != node.arguments.end(); ++it) {
            (*it)->accept(eval);
            if (std::holds_alternative<int>(eval.result)) {
                std::cout << std::get<int>(eval.result) << " ";
            } else if (std::holds_alternative<double>(eval.result)) {
                std::cout << std::get<double>(eval.result) << " ";
            } else if (std::holds_alternative<std::string>(eval.result)) {
                std::cout << std::get<std::string>(eval.result) << " ";
            } else {
                std::cout << "INVALID";
            }
        }
